/**
 * aiBinaryGenome.js — Pulse OS v11‑EVO Organ
 * ---------------------------------------------------------
 * CANONICAL ROLE:
...
class AIBinaryGenome {
  ...
  _computeGeneticThroughput(organCount, driftCount) {
    const driftFactor = Math.min(1, driftCount / organCount);
  ...
  generateGenome() {
    ...
    for (const id of organIds) {
      const signature = this.evolution.loadSignature({ id }) || '0';
      signatures[id] = signature;
      signatureBits += signature.length;

      const stored = this.evolution.loadSignature({ id });
      const current = signature;
      if (stored && stored !== current) driftCount++;
    }
