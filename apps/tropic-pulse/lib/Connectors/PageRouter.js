// ============================================================================
// FILE: /apps/tropic-pulse/lib/Connectors/PageRouter.js
// LAYER: A2‑LAYER (PAGE‑LEVEL PATHWAY)
//
// PURPOSE:
// This file is a PASS‑THROUGH PATHWAY between the Page (A‑Layer) and the
// Frontend Connector (B‑Layer). It does NOT contain logic, identity handling,
// backend rules, or business decisions.
//
// The PageRouter ONLY:
// • Catches page‑level missing‑field errors.
// • Extracts the Firestore table + field name.
// • Sends that information to Router.js for secure handling.
//
...
window.addEventListener("error", async (event) => {
  ...
  console.warn("[PageRouter] Missing:", table + "." + field);
  ...
  try {
    ...
    console.error("[PageRouter] Router fetch failed:", err);
  }
...
// This file assumes:
// • All missing fields come from the Users table.
...
function parseMissingField(message) {
  ...
  if (match) {
    return { table: "Users", field: match[1] };
  }
