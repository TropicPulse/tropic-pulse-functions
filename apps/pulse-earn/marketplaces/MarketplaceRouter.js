// ============================================================================
// FILE: tropic-pulse-functions/apps/pulse-earn/marketplaces/MarketplaceRouter.js
// LAYER: THE CONSULATE (Cross‑Marketplace Intelligence + Job Prioritizer)
// ============================================================================
//
// ROLE:
...
//   • READ‑WRITE only to its own in‑memory routerState
...
// ---------------------------------------------------------------------------
import { RegisteredMarketplaces } from "./RegisteredMarketplaces.js";
...
function extractFactors(job) {
  ...
  const cpuBand = job.cpuRequired != null
    ? `cpu:${Math.round(Number(job.cpuRequired) || 0)}`
    : null;

  const memBand = job.memoryRequired != null
    ? `mem:${Math.round(Number(job.memoryRequired) || 0)}`
    : null;

  const timeBand = job.estimatedSeconds != null
    ? `time:${Math.round(Number(job.estimatedSeconds) || 0)}`
    : null;

  const gpuBand = job.minGpuScore != null
    ? `gpu:${Math.round(Number(job.minGpuScore) || 0)}`
    : null;

  const bwBand = job.bandwidthNeededMbps != null
...
function indexFactors(fingerprint, factors) {
  factors.forEach(f => {
    if (!routerState.factorIndex.has(f)) {
      routerState.factorIndex.set(f, new Set());
    }
    routerState.factorIndex.get(f).add(fingerprint);
  });
}
...
function computePriorityScore(job) {
  ...
  const profile = getMarketplaceProfile(job.marketplaceId || job._sourceMarketplaceId || "unknown");
...
function updateMarketplaceStats(jobs) {
  const map = routerState.marketplaceStats;
  ...
  for (const [id, list] of grouped.entries()) {
    ...
    const prev = map.get(id) || { jobsSeen: 0, avgSlope: 0, lastCycleJobs: 0 };
...
function processJobsIntelligently(jobs) {
  const uniqueJobs = [];
  const { stats, resultCache, fingerprintIndex } = routerState;
...
function recordJobResult(job, result) {
  ...
  routerState.resultCache.set(fp, {
    result,
    ts: Date.now(),
...
// ---------------------------------------------------------------------------
// Public: Read-Only Router Healing / Intelligence State
// ---------------------------------------------------------------------------
function getMarketplaceRouterHealingState() {
  const marketplaceStats = {};
  for (const [id, value] of routerState.marketplaceStats.entries()) {
    marketplaceStats[id] = { ...value };
  }

  return {
    stats: { ...routerState.stats },
    resultCacheSize: routerState.resultCache.size,
    fingerprintCount: routerState.fingerprintIndex.size,
    factorKeyCount: routerState.factorIndex.size,
    marketplaceStats
  };
}

// ============================================================================
// Exported API — MARKETPLACE ROUTER (The Consulate)
// ============================================================================
export const MarketplaceRouter = {
  ...
  // Diagnostics / healing
  getHealingState: getMarketplaceRouterHealingState
};
